...
if __name__  == "__main__":
    ...
    # ### Get cosine similarities between a monolingual sentence and its translation.
    # Commented out as we don't use it for eval in the replication.
    #
    # out_dir = os.path.join(args.output_dir, "language_similarities")
    # out_file = os.path.join(out_dir, "similarities.csv")
    # if not os.path.exists(out_dir):
    #     os.mkdir(out_dir)
    
    # cs_en_scores = get_similarity_scores(model, cs_maj_en, "cs_sentences", "en_sentences", f"{out_dir}/cs_maj_en-cs_en")
    # cs_es_scores = get_similarity_scores(model, cs_maj_en, "cs_sentences", "es_sentences", f"{out_dir}/cs_maj_en-cs_es")
    # cs_es_scores = get_similarity_scores(model, cs_maj_en, "en_sentences", "es_sentences", f"{out_dir}/cs_maj_en-en_es")

    # cs_en_scores = get_similarity_scores(model, cs_maj_es, "cs_sentences", "en_sentences", f"{out_dir}/cs_maj_es-cs_en")
    # cs_es_scores = get_similarity_scores(model, cs_maj_es, "cs_sentences", "es_sentences", f"{out_dir}/cs_maj_es-cs_es")
    # cs_es_scores = get_similarity_scores(model, cs_maj_es, "en_sentences", "es_sentences", f"{out_dir}/cs_maj_es-en_es")
    
    # cs_en_scores = get_similarity_scores(model, cs_random, "cs_sentences", "en_sentences", f"{out_dir}/cs_random-cs_en")
    # cs_es_scores = get_similarity_scores(model, cs_random, "cs_sentences", "es_sentences", f"{out_dir}/cs_random-cs_es")
    # cs_es_scores = get_similarity_scores(model, cs_random, "en_sentences", "es_sentences", f"{out_dir}/cs_random-en_es")
    ...
    a_s = get_similarity_scores(model, a, "sentence1", "sentence2", args.output_dir)
    b_s = get_similarity_scores(model, b, "sentence1", "sentence2", args.output_dir)
    get_spearman_rank_correlation(a_s, b_s, out_dir, "Correlation between en & cs in a majority cs-es corpus", "en similarities", "cs similarities")
